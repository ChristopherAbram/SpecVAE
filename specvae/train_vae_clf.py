import sys
from dataset import DeUpscaleIntensity
import torch
import numpy as np
...
from vae import JointSpecGaussianVAEandClassifier
from train import JointVAEandClassifierTrainer
...
def main(argc, argv):
    ...
    # Column settings:
    target_column_name      = 'ionization_mode'
    target_column           = target_column_name + '_id'
    ...
    layers_configs          = [
        ...
        # lambda indim: [[indim, 80, 20], [20, 80, indim]],
    ...
    for max_num_peaks, min_intensity in pre_configs:
        configs = it.product(layers_configs, clf_layers_configs, learning_rates, dropouts)
        ...
        for i, (layers_fn, clf_layers_fn, learning_rate, dropout) in enumerate(configs):
            indim = 2 * max_num_peaks
            layers = layers_fn(indim)
            clf_layers = clf_layers_fn(layers[0][-1])
            ...
            # Create model:
            model = JointSpecGaussianVAEandClassifier(config, device)
            ...
            trainer = JointVAEandClassifierTrainer(model, writer)
            trainer.compile(
                optimizer=optim.Adam(model.parameters(), lr=learning_rate),
                metrics=['loss', 'kldiv', 'recon', 'clf_loss'],
                evaluation_metrics=['cos_sim'],
                evaluation_metrics_sub=['balanced_accuracy_score'])
