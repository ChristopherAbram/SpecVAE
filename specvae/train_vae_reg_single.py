...
from specvae.vae import VAEandRegressor
from specvae.train import VAEandRegressorTrainer


def main(argc, argv):
    # Set and parse arguments:
    parser = argparse.ArgumentParser(description='Train VAE model jointly with regressor for metabolomics data')
    ...
    parser.add_argument('--model-name', type=str, help='A name of model', default='betavae_reg')
    parser.add_argument('--dataset', type=str, choices=['MoNA', 'HMDB'], help='Name of a dataset used for training', default='MoNA')
    parser.add_argument('--n-samples', type=int, help='Number of samples used in training, -1 takes all available training samples', default=2000)
    ...
    parser.add_argument('--n-epochs', type=int, help='Training parameter, number of training epochs', default=10)
    ...
    parser.add_argument('--input-columns', type=str, help='Columns from dataset used as an input for classifier', 
        default='["spectrum"]')
    parser.add_argument('--target-column', type=str, help='Name of the target column for classification model', default='collision_energy')
    ...
    reg_layer_config        = lambda indim: [indim, int(indim / 1.5)]
    ...
    reg_layers = reg_layer_config(indim)
    ...
    if resume:
        try:
            ...
            if os.path.exists(filepath):
                ...
                # Find configuration:
                res = df[
                    (df['reg_target_column'].isin(          [target_column])) & 
    ...
    train_loader, valid_loader, test_loader, metadata = dt.load_data_regression(
        ...
        input_columns, types, target_column, True, view=dt.JointTrainingDataset)
    ...
    config = {
        ...
        'types':                [torch.float32],
        'reg_target_column':    target_column,
        'reg_input_columns':    input_columns,
        'reg_input_sizes':      input_sizes,
        'reg_layer_config':     reg_layers,
        'regressor_config':     {
            'layer_config':         reg_layers,
            'dropout':              0.,
    ...
    try:
        # Create model:
        model = VAEandRegressor(config, device)
        ...
        trainer = VAEandRegressorTrainer(model, writer)
        trainer.compile(
        ...
        train.export_training_session(trainer, paths, 
            ...
            evaluation_metrics_sub=['MSE', 'RMSE', 'MAE', 'R2', 'explained_variance'])
